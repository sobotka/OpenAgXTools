...
def ocio_set_displays(config, foo):
    return True


def ocio_create_allocation_transform(
    minValue,
    maxValue,
    type,
    offset=None,
...
def ocio_matrix33_to_flat44(
    ...
    flat44 = matrix44.flatten()
    flat44[-1] = 1.
    return flat44


def ocio_write_spi1d(
    filename,
    values,
    from_minimum=0.,
    from_maximum=1.,
...
def ocio_write_config(subdirectory, config, prefix=None):
    try:
        config.sanityCheck()
